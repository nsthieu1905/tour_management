...
class CustomerNotificationManager {
  constructor() {
    ...
    this.socket = null;
    this.customerId = this.getCustomerIdFromDom();
    this.init();
  }
  ...
  // Lấy customer ID từ DOM hoặc tạo một ID ngẫu nhiên
  getCustomerIdFromDom() {
    const customerId =
      localStorage.getItem("customerId") ||
      document.body.dataset.customerId ||
      document.querySelector("[data-customer-id]")?.dataset.customerId;
    return customerId || "customer_" + Math.random().toString(36).substr(2, 9);
  }
  ...
  initializeSocket() {
    ...
    this.socket.on("connect", () => {
      // Gửi customer:join cho thông báo chung
      this.socket.emit("customer:join", this.customerId);
...
if (typeof module !== "undefined" && module.exports) {
  module.exports = CustomerNotificationManager;
}
