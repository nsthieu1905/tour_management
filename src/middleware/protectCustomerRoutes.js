...
const buildCustomerLoginUrl = (nextUrl) => {
  const safeNext = nextUrl || "/";
  return `/customer/auth/login?next=${encodeURIComponent(safeNext)}`;
};
...
const respondAuthRequired = (req, res) => {
  const nextUrl = isApiRequest(req)
    ? req.headers.referer || "/"
    : req.originalUrl || "/";
  const loginUrl = buildCustomerLoginUrl(nextUrl);
...
const handleCustomerAuth = async (req, res, next, { optional } = {}) => {
  try {
...
const protectCusRoutes = (req, res, next) =>
  handleCustomerAuth(req, res, next, { optional: false });

protectCusRoutes.optional = (req, res, next) =>
  handleCustomerAuth(req, res, next, { optional: true });

module.exports = protectCusRoutes;
